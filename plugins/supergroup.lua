...
local function run(msg, matches)
	...
	local name_log = print_name:gsub("_", " ")
	local data = load_data(_config.moderation.data)
			if matches[1] == 'aded' and not matches[2] then
		...
		if matches[1] == 'aded' and is_admin1(msg) and not matches[2] then
		...
		if matches[1] == 'remo' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
		...
		if matches[1] == 'p' and is_owner(msg) then
			local chat_id = msg.to.id
